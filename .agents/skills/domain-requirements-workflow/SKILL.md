...
Personas are a source of questions and hypotheses, never a source of evidence (see Red flags). Use them to expose what to ask and where the risky unknowns are, not to manufacture answers. Personas cover org-role perspectives; for general-domain background expertise (industry standards, regulations, common practice), dispatch the `domain-expert-agent` instead — see stage 6. When that background spans distinct areas (e.g. medical regulation vs. accounting practice), dispatch one `domain-expert-agent` per area rather than one agent for all of them — each agent is scoped to a single target domain.
